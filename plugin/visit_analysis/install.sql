...
CREATE TABLE `{pre}visit_analysis` (
	...
	`host` Char(100) NOT NULL UNIQUE COMMENT '来源主机',
	`count` MEDIUMINT UNSIGNED DEFAULT 0 NOT NULL COMMENT '总来访次数',
	`count_month` MEDIUMINT UNSIGNED DEFAULT 0 NOT NULL COMMENT '月来访次数',
	`count_year` MEDIUMINT UNSIGNED DEFAULT 0 NOT NULL COMMENT '年来访次数',
	`add_date` Char(15) DEFAULT 0 COMMENT '首次来访日期（unixtimestamp）',
	`chg_date` Char(15) DEFAULT 0 COMMENT '最近来访日期（unixtimestamp）',
	index (`host`),
	PRIMARY KEY (`id`)
...
CREATE TABLE `{pre}visit_keyword` (
	`id` MEDIUMINT UNSIGNED NOT NULL AUTO_INCREMENT,
	`keyword` Char(200) NOT NULL UNIQUE COMMENT '关键字',
	`count` SMALLINT DEFAULT 0 COMMENT '搜索次数',
	`url` Char(200) DEFAULT "###" NOT NULL COMMENT '访问网址',
	`referer` Char(255) DEFAULT "" NOT NULL COMMENT '最近来源网址',
