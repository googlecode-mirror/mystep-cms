...
function removeItem(item) {
	...
	switch(reg_item[item].op) {
		case "op_add":
			delete reg_item[item];
			break;
		case "op_edit":
		case undefined:
			reg_item[item].op = "op_remove";
			break;
		default:
			reg_item[reg_item[item].op] = new Object();
			reg_item[reg_item[item].op].op = "op_remove";
			delete reg_item[item];
			break;
	}
	$("#item_"+item).remove();
...
function confirmItem(mode) {
	switch(mode) {
		...
		case "edit":
			...
			if(idx=="") return;
			if(idx!=idx_org) {
				switch(reg_item[idx_org].op) {
					case "op_add":
						delete reg_item[idx_org];
						new_item.op = "op_add";
						break;
					case "op_edit":
					case undefined:
						...
						break;
					default:
						new_item.op = reg_item[idx_org].op;
						delete reg_item[idx_org];
						break;
				}
			} else {
				new_item.op = (reg_item[idx_org].op!=undefined)?reg_item[idx_org].op:"op_edit";
				delete reg_item[idx];
			}
			reg_item[idx] = new_item;
