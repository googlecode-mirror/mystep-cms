...
function snatchGetInfo($url, $para=array()) {
	...
	$url = "http://it.sohu.com/_scroll_newslist/".$date."/news.inc";
	$info = array();
	$info['page_count'] = 1;
	$header = array();
	...
	if($content = GetRemoteContent($url, $header)) {
		...
		unset($content);
		$info['cat_main'] = $db->getSingleResult("select cat_id from ".$setting['db']['pre']."news_cat where cat_name='IT'");
		if(empty($info['cat_main'])) {
			$db->Query("INSERT INTO `".$setting['db']['pre']."news_cat` VALUES (0, ".$para['web_id'].", 0, 'IT', 'IT,IT新闻', '全部互联网、通信、IT业界、科学资讯', 'IT', '', '', 1, 0, '', 1, 255, 0, '');");
...
function snatchGetList($record, &$info) {
	...
	for($i=0, $m=count($info['newList']); $i<$m; $i++) {
		...
		if(strpos($record['url'], "http://it.sohu.com")===false) continue;
