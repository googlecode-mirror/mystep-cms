<?php
$info = array(
	"name" => "Offical Plugin",
	"intro" => "Offical Plugin Show, you may treat it as an example.",
	"copyright" => "Copyright 2010 <a href='mailto:windy2006@gmail.com'>Windy2000</a>",
	"description" => "<b>With this plugin, the function template tags you can use list as below£º</b>
<b>News: </b>Show Article list in the page from table 'news_show' of database. Attribute include: web_id, cat_id, order, setop, show_image, css1, css2, limit, loop, show_catalog, show_date, tag, template
<b>Info: </b>Show Custom HTML content in the page from table 'info_show' of database. Attribute include: id, title
<b>Link: </b>Show external links in the page from table 'link' of database. Attribute include: type, limit, idx
<b>Tag: </b>Show news tags in the page from table 'news_tag' of database. Attribute include: template, limit, order
<b>Include: </b>Include server side file in the specified position of the page. Attribute include: file"
);
?>