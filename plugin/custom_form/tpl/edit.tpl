...
<!--loop:end-->
			...
			<tr>
				...
				<td class="row">
					<input name="notes" type="text" value="<!--notes-->" maxlength="150" />
				</td>
			</tr>
			<tr>
				<td colspan="2">
					<table width="100%">
						<tr><td class="cat" colspan="4">表单项目 &nbsp; [<a href="###" onclick="addItem()">添加</a>] &nbsp; [<a href="###" onclick="exportItem()">导出</a>]</td></tr>
...
function removeItem(item) {
	...
	switch(cf_item[item].op) {
		case "op_add":
			delete cf_item[item];
			break;
		case "op_edit":
		case undefined:
			cf_item[item].op = "op_remove";
			break;
		default:
			cf_item[cf_item[item].op] = new Object();
			cf_item[cf_item[item].op].op = "op_remove";
			delete cf_item[item];
			break;
	}
	$("#item_"+item).remove();
...
function confirmItem(mode) {
	switch(mode) {
		...
		case "edit":
			...
			if(idx=="") return;
			if(idx!=idx_org) {
				switch(cf_item[idx_org].op) {
					case "op_add":
						delete cf_item[idx_org];
						new_item.op = "op_add";
						break;
					case "op_edit":
					case undefined:
						...
						break;
					default:
						new_item.op = cf_item[idx_org].op;
						delete cf_item[idx_org];
						break;
				}
			} else {
				new_item.op = (cf_item[idx_org].op!=undefined)?cf_item[idx_org].op:"op_edit";
				delete cf_item[idx];
			}
			cf_item[idx] = new_item;
