<?php
class plugin_source implements plugin {
	public static function install() {
		...
		$db->query('insert into '.$setting['db']['pre'].'plugin VALUES (0, "'.$info['name'].'", "'.$info['idx'].'", "'.$info['ver'].'", "plugin_source", 1, "'.$info['intro'].'", "'.$info['copyright'].'", 1, "")');
		$err = array();
		if($db->GetError($err)) {
	...
	public static function check() {
		global $setting;
		$result = "";
		if($setting['rewrite']['enable']) $result = "Current Plugin cannot be run under rewrite mode!".'<span id="error"></span>';
		return $result;
	}
}
?>