...
CREATE TABLE `{pre}news_show` (
	...
	`image` Char(100) NOT NULL DEFAULT '' COMMENT '相关图片',
	`setop` SMALLINT UNSIGNED COMMENT '置顶模式',
...
CREATE TABLE `{pre}news_detail` (
	...
	`sub_title` Char(100) DEFAULT '' COMMENT '子标题',
	`ctype` TINYINT UNSIGNED DEFAULT 1 COMMENT '内容类型(ubb, html)',
