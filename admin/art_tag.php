...
switch($method) {
	...
	case "rebuild":
		...
		$db_tmp->Query("update ".$setting_sub['db']['name'].$setting_sub['db']['pre']."news_tag set `count`=0");
		$db->ReConnect(true);
		$db->Query("select news_id, tag from ".$setting_sub['db']['name'].$setting_sub['db']['pre']."news_show order by news_id");
		while($record = $db->GetRS()) {
			...
			for($n=0; $n<$max_count; $n++) {
				...
				if(strlen($the_tag[$n]>24)) {
					...
					$db_tmp->Query("update ".$setting_sub['db']['name'].$setting_sub['db']['pre']."news_show set tag = '".$the_tag[$n]."' where news_id='{$record['news_id']}'");
				}
				if($db_tmp->GetSingleResult("select id from ".$setting_sub['db']['name'].$setting_sub['db']['pre']."news_tag where `tag` = '".$the_tag[$n]."'")) {
					$db_tmp->Query("update ".$setting_sub['db']['name'].$setting_sub['db']['pre']."news_tag set `count` = `count` + 1, update_date = UNIX_TIMESTAMP() where `tag` = '".$the_tag[$n]."'");
				} else {
					$db_tmp->Query("insert into ".$setting_sub['db']['name'].$setting_sub['db']['pre']."news_tag values(0, '".$the_tag[$n]."', 1, 0, UNIX_TIMESTAMP(), UNIX_TIMESTAMP())");
				}
			}
		}
		$db_tmp->Query("delete from ".$setting_sub['db']['name'].$setting_sub['db']['pre']."news_tag where `count`<2 and `click`<5 and `add_date`<UNIX_TIMESTAMP()-60*60*24*10");
		$db->Free();
		$db->Query("select id, tag from ".$setting_sub['db']['name'].$setting_sub['db']['pre']."news_tag");
		while($record = $db->GetRS()) {
			$counter = $db_tmp->GetSingleResult("select count(*) from ".$setting_sub['db']['name'].$setting_sub['db']['pre']."news_show where tag like '%{$record['tag']}%'");
			$db_tmp->Query("update ".$setting_sub['db']['name'].$setting_sub['db']['pre']."news_tag set `count`='{$counter}' where id='{$record['id']}'");
		}
		$db_tmp->Close();
		unset($db_tmp);
		break;
...
function build_page($method) {
	...
	$str_sql = "select count(*) as counter from ".$setting_sub['db']['name'].$setting_sub['db']['pre']."news_tag where 1=1";
	...
	$str_sql = "select * from ".$setting_sub['db']['name'].$setting_sub['db']['pre']."news_tag where 1=1";
	if(!empty($keyword)) $str_sql.= " and tag like '%{$keyword}%'";
